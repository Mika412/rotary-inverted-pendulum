...
| Motor angle quantisation (firmware model only) | snapped to step LSB (2π / 3200 rad at 1/16) | `MOTOR_STEP_LSB_RAD`   | The firmware's motor position is the step counter. Derived from `MOTOR_MICROSTEPS`, which must match the `MICROSTEPS` constant in the sketches (16 on the TMC2209, 8 on a DRV8825). Applied at measurement capture when `firmware_obs_model` is on. |
