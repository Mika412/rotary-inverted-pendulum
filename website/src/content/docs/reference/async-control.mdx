...
import AsyncThreadsDiagram from '../../../components/AsyncThreadsDiagram.astro';
import constants from '../../../generated/constants.json';

How `finetune_async.py` holds the rig's control loop at exactly the rate it was
configured for — {constants.control.frequencyHz} Hz as the stack currently
stands — while SAC's gradient updates run in parallel.
...
blocks on the rig**. They communicate through two shared objects, each
lock-protected and each flowing one way: transitions out to the learner, and
refreshed policy weights back. The replay buffer is not one of them — it never
leaves the learner.
...
Adds under 1% CPU at 100 Hz, pulls jitter from ~2 ms down to below 0.1 ms.
