...
| 3D-printed parts      | Arm, base, lid, pendulum link (with embedded coin), encoder boss | ~£2.00 (filament) | —                                                                   | Self-printed. <100 g PLA at ~£20/kg. STL files in `meshes/`; OnShape source linked from the project README. The whole assembly is **press-fit** — no fasteners required. Pendulum link mass, COM, and inertia tensor live in `urdf/model.urdf` (exported from Onshape with PLA-density-corrected materials) — the RL sim and the sysid pipeline both read from there. |
